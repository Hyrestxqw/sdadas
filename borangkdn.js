    ...
    if(interaction.isButton()) {
      ...
      if(interaction.customId === "etkinlik") {
        ...
        if(member.roles.cache.has(config.buttons.etkinlik)) {
          ...
          await interaction.reply({ content: `<@&${config.buttons.etkinlik}> rolünü üzerinizden başarıyla aldım! Bu Yüzden Kanala İznini Kapattım`, ephemeral: true });
        } else {
          await member.roles.add(config.buttons.etkinlik);
          await interaction.reply({ content: `<@&${config.buttons.etkinlik}> rolünü üzerinize başarıyla ekledim! <#984815816719630346> Kanalından Altyapılara Erişe Bilirsin`, ephemeral: true });
        };
      };
      ...
      if(interaction.customId === "cekilis") {
        ...
        if(member.roles.cache.has(config.buttons.çekiliş)) {
          ...
          await interaction.reply({ content: `<@&${config.buttons.çekiliş}> rolünü üzerinizden başarıyla aldım! Bu Yüzden Kanala İznini Kapattım`, ephemeral: true });
        } else {
          await member.roles.add(config.buttons.çekiliş);
          await interaction.reply({ content: `<@&${config.buttons.çekiliş}> rolünü üzerinize başarıyla ekledim! <#984815816719630346> Kanalından Kodlara Erişe Bilirsin`, ephemeral: true });
        };
      };
